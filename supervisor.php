...
try {
    ...
    $supervisorStmt = $conn->prepare("SELECT username FROM Users WHERE id = ?");
    $supervisorStmt->execute([$_SESSION["userId"]]);
    $supervisorName = $supervisorStmt->fetch(PDO::FETCH_COLUMN);

    $stmt = $conn->prepare("SELECT * FROM task_assignments WHERE staff_member = ?");
